...
class KlasemenController extends CI_Controller
{
    ...
    {
        $this->load->view('input_club_view');
    }

    public function process_input_club_view()
    {
        $this->load->library('session');
    ...
    public function input_score_view()
    {
        $data['clubs'] = $this->KlasemenModel->get_clubs();
        $this->load->view('input_score_view', $data);
    }
    public function input_score_multiple_view()
    {
        $data['clubs'] = $this->KlasemenModel->get_clubs();
        $this->load->view('input_score_multiple_view', $data);
    }
    // Klasemen.php
    ...
    public function process_input_score_multiple()
    {
        ...
        // Cek apakah data input merupakan array atau objek Countable sebelum menggunakan count()
        if (
            !is_array($club1_ids) || !is_array($club2_ids) ||
            !is_array($score1_values) || !is_array($score2_values)
        ) {
            $this->session->set_flashdata('error', 'Data input tidak valid!');
            redirect('KlasemenController/input_score_multiple_view');
        }
